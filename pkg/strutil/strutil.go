...
func Parse2UnixPath(str string) (res string) {
	if !IsBlank(str) {
		res = path.Clean(strings.ReplaceAll(str, "\\", "/"))
	} else {
		res = "/"
	}
	return
...
// RemoveDotSegmentsAndParse2UnixPath 删除路径后面 /, 把\转换为/, 删除./和../
// 例如: ./a/b/../c/ => /a/c
func RemoveDotSegmentsAndParse2UnixPath(str string) string {
	if len(str) > 0 {
...
func GetPathParent(path string) string {
	if path = Parse2UnixPath(path); path == "/" {
		return path
	}
...
// FindMissing 查询input不在src中的数据
func FindMissing(src, input []string) []string {
	if len(src) == 0 || len(input) == 0 {
		return input
	}

	bSet := make(map[string]any, len(src))
...
func GetRandom(l int) string {
	...
	for i := 0; i < l; i++ {
		randMu.Lock()
		result = append(result, bytes[rnd.Intn(len(bytes))])
		randMu.Unlock()
	}
...
func RemoveDuplicatesAndEmpty(input ...string) []string {
	result := make([]string, 0, len(input))
	if len(input) > 0 {
		uniqueMap := make(map[string]any, len(input))
...
// ArrayFilter 接收一个切片和一个过滤条件函数，返回一个新的切片，包含所有满足条件的元素
func ArrayFilter[T any](array []T, test func(T) bool) []T {
	var result []T
	for i := 0; i < len(array); i++ {
		if test(array[i]) {
